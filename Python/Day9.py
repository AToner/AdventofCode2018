...
class MarbleTracker:
    ...
    def __str__(self):
        result = []
        head = self.first_marble
        for _ in range(self.marble_count):
            result.append(head.value)
            head = head.next_marble

        return str(result)

    def store_marble(self, marble_value):
        marble = Marble(marble_value)
        if self.current_marble is None:
            marble.next_marble = marble
            marble.prev_marble = marble
            self.first_marble = marble
        else:
            prev = self.current_marble.prev_marble
            prev.next_marble = marble
            marble.prev_marble = prev
            marble.next_marble = self.current_marble
            self.current_marble.prev_marble = marble
    ...
    def remove_marble(self):
        prev = self.current_marble.prev_marble
        next_marble = self.current_marble.next_marble

        prev.next_marble = next_marble
        next_marble.prev_marble = prev
    ...
    def clockwise(self, n):
        for _ in range(n):
            self.current_marble = self.current_marble.next_marble

    def counter_clockwise(self, n):
        for _ in range(n):
            self.current_marble = self.current_marble.prev_marble


def play_game(players, last_marble):
    board = MarbleTracker()
    number_players = players
    scores = [0 for _ in range(number_players + 1)]
    lowest_marble = 1
    largest_marble = last_marble

    board.store_marble(0)
    current_player = 1
    while lowest_marble <= largest_marble:
        if lowest_marble % 23 == 0:
            scores[current_player] += lowest_marble
            board.counter_clockwise(7)
            scores[current_player] += board.current_marble.value
            board.remove_marble()
        else:
            board.clockwise(2)
            board.store_marble(lowest_marble)
...
# Part One... Test
print(play_game(9, 25))  # 32
print(play_game(10, 1618))  # 8317
print(play_game(13, 7999))  # 146373
print(play_game(17, 1104))  # 2764
print(play_game(21, 6111))  # 54718
print(play_game(30, 5807))  # 37305

# Part One
# 493 players; last marble is worth 71863 points
print(play_game(493, 71863))  # 367802

# Part Two
start = datetime.now()
print(play_game(493, 7186300))  # 2996043280
end = datetime.now()
print(end - start)
