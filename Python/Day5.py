...
def react_input(input):
    result = input
    combos = []
    chars = set(list(input.strip().lower()))
    for char in chars:
        combos.append(char.lower() + char.upper())
        combos.append(char.upper() + char.lower())

    made_change = True
    while made_change:
        length_before = len(result)
        for combo in combos:
            result = result.replace(combo, "")
        made_change = length_before != len(result)
...
def part_two(input):
    ...
    best_length = len(input)

    for char in chars:
        result = input.replace(char.lower(), "")
        result = result.replace(char.upper(), "")
        result = react_input(result)
        best_length = min(len(result), best_length)

    return best_length


def part_one(input):
    return len(react_input(input))


with open("Day5.txt") as file:
    file_text = file.readlines()

print(part_one('dabAcCaCBAcCcaDA'))
print(part_two('dabAcCaCBAcCcaDA'))
